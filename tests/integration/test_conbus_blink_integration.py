...
class TestConbusBlinkIntegration:
    ...
    def _create_mock_conbus_service(self, discover_devices=None, discover_success=True, blink_success=True):
        ...
        discover_telegrams = ["\n".join([f"<R{device}F01DFI>" for device in discover_devices])] if discover_success else []
        discover_response = self._create_mock_conbus_response(
            success=discover_success,
            telegrams=discover_telegrams
        )
    ...
    def test_conbus_blink_invalid_response(self, mock_conbus_service):
        ...
        assert response.success is True  # Success because discover worked, just no devices
        assert response.serial_number == "all"
        assert response.operation == "off"
        assert response.system_function == SystemFunction.UNBLINK
