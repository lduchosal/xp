...
from src.xp.services.conbus_client_send_service import ConbusClientSendService, ConbusClientSendError
...
class TestTelegramGeneration(TestConbusClientSendService):
    ...
    def test_version_telegram_generation(self, service):
        ...
        with patch.object(service.version_service, 'generate_version_request_telegram') as mock_version_method:
            mock_response = Mock()
...
class TestTelegramSending(TestConbusClientSendService):
    ...
    @patch('socket.socket')
    def test_telegram_send_connection_failure(self, mock_socket_class, service, mock_socket):
    ...
    def test_telegram_send_generation_failure(self, mock_socket_class, service):
        ...
        request = ConbusSendRequest(telegram_type=TelegramType.VERSION)  # Missing serial
    ...
    @patch('socket.socket')
    def test_telegram_send_socket_timeout(self, mock_socket_class, service, mock_socket):
...
class TestErrorHandling(TestConbusClientSendService):
    ...
    @patch('socket.socket')
    def test_service_resilience_to_socket_errors(self, mock_socket_class, service, mock_socket):
