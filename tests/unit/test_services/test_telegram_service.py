...
from src.xp.models.event_telegram import EventType, InputType, EventTelegram
...
class TestTelegramService:
    ...
    def test_format_telegram_summary(self):
        ...
        assert "Event: XP2606 (Type 14) Link 00 Input 02 (push_button) pressed" in summary
...
class TestSystemTelegramParsing:
    ...
    def test_parse_valid_system_telegram(self):
        ...
        assert result.system_function == SystemFunction.RETURN_DATA
    ...
    def test_parse_system_telegram_different_functions(self):
        ...
        result = self.service.parse_system_telegram(raw)
        assert result.system_function == SystemFunction.UPDATE_FIRMWARE
    ...
    def test_parse_system_telegram_different_data_points(self):
        ...
        result = self.service.parse_system_telegram(raw)
        assert result.data_point_id == DataPointType.STATUS
    ...
    def test_parse_system_telegram_invalid_format(self):
        ...
        with pytest.raises(TelegramParsingError, match="Invalid system telegram format"):
            self.service.parse_system_telegram("<S002001252F02D18FN>")  # Wrong serial number length
        
        with pytest.raises(TelegramParsingError, match="Invalid system telegram format"):
            self.service.parse_system_telegram("<S0020012521F2D18FN>")  # Wrong function format
        
        with pytest.raises(TelegramParsingError, match="Invalid system telegram format"):
            self.service.parse_system_telegram("<S0020012521F02D8FN>")  # Wrong data point format
    
    def test_parse_system_telegram_unknown_function(self):
        """Test parsing system telegram with unknown function code"""
        with pytest.raises(TelegramParsingError, match="Unknown system function code: 99"):
    ...
    def test_format_system_telegram_summary(self):
        ...
        assert "System: System Telegram: Return Data for Temperature from device 0020012521" in summary
...
class TestReplyTelegramParsing:
    ...
    def test_parse_reply_telegram_invalid_format(self):
        ...
        with pytest.raises(TelegramParsingError, match="Invalid reply telegram format"):
            self.service.parse_reply_telegram("<R002001252F02D18+26,0§CIL>")  # Wrong serial number length
        
        with pytest.raises(TelegramParsingError, match="Invalid reply telegram format"):
            self.service.parse_reply_telegram("<R0020012521F2D18+26,0§CIL>")  # Wrong function format
    ...
    def test_format_reply_telegram_summary(self):
        ...
        assert "Reply: Reply Telegram: Data Response for Temperature = 26.0°C from device 0020012521" in summary
