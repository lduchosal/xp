...
class TestBlinkService:
    ...
    def test_generate_blink_telegram_valid(self):
        ...
        result = service.generate_blink_telegram("0020044964")
        assert result == "<S0020044964F05D00FN>"
        
        # Test another case
        result = service.generate_blink_telegram("0020030837")
        assert result == "<S0020030837F05D00FJ>"
        
        # Test different serial numbers
        result = service.generate_blink_telegram("1234567890")
    ...
    def test_generate_blink_telegram_invalid_serial(self):
        ...
        with pytest.raises(BlinkError, match="Serial number must be 10 digits"):
            service.generate_blink_telegram("")
        
        # Test short serial
        with pytest.raises(BlinkError, match="Serial number must be 10 digits"):
            service.generate_blink_telegram("123456789")
        
        # Test long serial
        with pytest.raises(BlinkError, match="Serial number must be 10 digits"):
            service.generate_blink_telegram("12345678901")
        
        # Test non-numeric serial
        with pytest.raises(BlinkError, match="Serial number must contain only digits"):
            service.generate_blink_telegram("123456789A")
    ...
    def test_generate_unblink_telegram_valid(self):
        ...
        result = service.generate_unblink_telegram("0020030837")
        assert result == "<S0020030837F06D00FK>"
        
        # Test another case
        result = service.generate_unblink_telegram("0020044964")
        assert result == "<S0020044964F06D00FO>"
        
        # Test different serial numbers
        result = service.generate_unblink_telegram("1234567890")
    ...
    def test_generate_unblink_telegram_invalid_serial(self):
        ...
        # Test empty serial
        with pytest.raises(BlinkError, match="Serial number must be 10 digits"):
            service.generate_unblink_telegram("")
        
        # Test short serial
        with pytest.raises(BlinkError, match="Serial number must be 10 digits"):
            service.generate_unblink_telegram("123456789")
        
        # Test long serial
        with pytest.raises(BlinkError, match="Serial number must be 10 digits"):
            service.generate_unblink_telegram("12345678901")
        
        # Test non-numeric serial
        with pytest.raises(BlinkError, match="Serial number must contain only digits"):
            service.generate_unblink_telegram("123456789A")
    ...
    def test_create_blink_telegram_object(self):
        ...
        assert telegram.data_point_id == DataPointType.STATUS
    ...
    def test_is_ack_response(self):
        ...
        other_reply.system_function = SystemFunction.RETURN_DATA
    ...
    def test_blink_unblink_telegram_format(self):
        ...
        blink_telegram = service.generate_blink_telegram("0020044964")
        ...
        unblink_telegram = service.generate_unblink_telegram("0020030837")
    ...
    def test_different_serial_numbers(self):
        ...
        for serial in test_serials:
            blink_telegram = service.generate_blink_telegram(serial)
            unblink_telegram = service.generate_unblink_telegram(serial)
