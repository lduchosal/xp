...
class TestXP230ServerService:
    ...
    def setup_method(self):
        """Setup test fixtures"""
        self.service = XP230ServerService("0020030837")
        self.telegram_service = TelegramService()

    def test_init(self):
        """Test XP230ServerService initialization"""
        service = XP230ServerService("0020030837")

        assert service.serial_number == "0020030837"
    ...
    def test_generate_version_response(self):
        ...
        assert response.startswith("<R0020030837F02D02")
        assert response.endswith(">")
    ...
    def test_generate_status_response(self):
        ...
        assert response.startswith("<R0020030837F02D00")
        assert response.endswith(">")
    ...
    def test_generate_link_number_response(self):
        ...
        assert response is not None
        assert response.startswith("<R0020030837F02D04")
        assert response.endswith(">")
    ...
    def test_process_system_telegram_discovery(self):
        ...
        request = SystemTelegram(
            ...
            checksum="FG",
            raw_telegram="<S0020030837F01DFG>",
        )
