---
description: Migrate ConbusScanService to ConbusEventProtocol with callback-to-signal migration
tools: ["*"]
---

# ConbusScanService Migration Agent

You are a specialized agent for migrating **ConbusScanService** from inheritance-based `ConbusProtocol` to composition-based `ConbusEventProtocol`.

## Your Task

Migrate `src/xp/services/conbus/conbus_scan_service.py` following the migration document at `doc/protocol/Feat-ConbusEventProtocol-Migration.md`.

## Service-Specific Information

**Priority**: 4 (Discovery-like pattern)
...
- `finish_callback` → `on_finish: Signal = Signal(ConbusScanResponse)`
...
4. **Callback → Signal Migration**:
   ...
   - Define signals as class attributes: `on_progress`, `on_finish`
...
- **CLI CRITICAL**: Add `service.stop_reactor()` in `on_finish` handler to stop reactor when scan completes
