...
def remove_highest_bit(value: int) -> int:
    ...
    """
    return value ^ 0x80 if (value & 0x80) == HIGH_BIT else value
...
def de_nibble(nibble_str: str) -> int:
    ...
    """
    if len(nibble_str) != NIBBLE_LENGTH:
        msg = "Nibble string must be exactly 2 characters"
...
def de_nibbles(str_val: str) -> bytearray:
    ...
    """
    if len(str_val) % 2 != 0:
        msg = "String length must be even for nibble conversion"
        raise ValueError(msg)
