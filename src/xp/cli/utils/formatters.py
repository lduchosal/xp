...
class OutputFormatter:
    ...
    def error_response(self, error: str, extra_data: Optional[Dict[str, Any]] = None) -> str:
...
class TelegramFormatter(OutputFormatter):
    ...
    def format_telegram_summary(self, telegram_data: Dict[str, Any], 
                               service_formatter_method=None) -> str:
    ...
    def format_validation_result(self, parsed_telegram: Any, checksum_valid: bool, 
                                service_summary: str) -> str:
...
class ListFormatter(OutputFormatter):
    ...
    def format_list_response(self, items: list, title: str, 
                           item_formatter=None) -> str:
    ...
    def format_search_results(self, matches: list, query: str) -> str:
        ...
        if self.json_output:
            return json.dumps({
                "query": query,
                "matches": [item.to_dict() if hasattr(item, 'to_dict') else item for item in matches],
                "count": len(matches)
            }, indent=2)
        ...
        for item in matches:
            if hasattr(item, 'code') and hasattr(item, 'name') and hasattr(item, 'description'):
...
class StatisticsFormatter(OutputFormatter):
    ...
    def format_file_statistics(self, file_path: str, stats: Dict[str, Any], 
                             entry_count: int) -> str:
