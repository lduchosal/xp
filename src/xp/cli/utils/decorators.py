...
def validation_option(func: Callable) -> Callable:
    ...
    return click.option('--validate-checksum', '-c', is_flag=True, 
                       help='Validate telegram checksum')(func)
...
def file_operation_command():
    ...
    def decorator(func: Callable) -> Callable:
        func = click.option('--time-range', 
                           help='Filter by time range (HH:MM:SS,mmm-HH:MM:SS,mmm)')(func)
        func = click.option('--filter-direction', 
                           type=click.Choice(['tx', 'rx']), 
                           help='Filter by direction')(func)
        func = click.option('--filter-type', 
                           type=click.Choice(['event', 'system', 'reply']), 
                           help='Filter by telegram type')(func)
        func = json_output_option(func)
        return func
    return decorator
