...
def _get_actiontable_type(xpmoduletype: str) -> ActionTableType:
    ...
    if xpmoduletype not in type_map:
        msg = f"Unsupported module type: {xpmoduletype}"
...
def conbus_show_msactiontable(ctx: Context, serial_number: str) -> None:
    ...
    def on_finish(module: ConsonModuleConfig) -> None:
        ...
        _echo_short_format(module)
        _echo_full_format(module)
...
def _echo_short_format(module: ConsonModuleConfig) -> None:
    """Display the short action table format if one is configured.

    Args:
        module: Module configuration to display.

    """
    table = (
        module.xp33_msaction_table
        or module.xp24_msaction_table
        or module.xp20_msaction_table
    )
    if table:
        click.echo("Short:")
        for line in table:
...
def _echo_full_format(module: ConsonModuleConfig) -> None:
    """Display the full YAML-like action table format.
