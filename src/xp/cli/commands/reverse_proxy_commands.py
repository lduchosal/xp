...
from ...services.conbus_reverse_proxy_service import ConbusReverseProxyService, ConbusReverseProxyError
...
def start_proxy(port: int, config: str, json_output: bool):
    ...
    try:
        ...
        _proxy_instance = ConbusReverseProxyService(config_path=config, listen_port=port)
        ...
        CLIErrorHandler.handle_service_error(e, json_output, "reverse proxy startup", {
            "port": port,
            "config": config
        })
    ...
    try:
        ...
        if json_output:
            ...
            if status_data.get("running", False):
                ...
                click.echo(f"  Target server: {status_data.get('target_ip')}:{status_data.get('target_port')}")
                click.echo(f"  Active connections: {status_data.get('active_connections', 0)}")
                ...
                if connections:
                    ...
                    for conn_id, conn_info in connections.items():
                        ...
                        click.echo(f"    {conn_id}: {client_addr} (connected: {connected_at}, {bytes_relayed} bytes)")
            else:
                click.echo("✗ Reverse proxy is not running")
                
    except Exception as e:
        CLIErrorHandler.handle_service_error(e, json_output, "reverse proxy status check")