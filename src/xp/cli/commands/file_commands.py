...
from ..utils.decorators import json_output_option, file_operation_command, handle_service_errors
    ...
    try:
        ...
        if filter_type or filter_direction or time_range:
            ...
            if time_range:
                try:
                    ...
                    error_response = OutputFormatter(json_output).error_response(f"Invalid time range: {e}")
                    if json_output:
        ...
        if summary:
            ...
            if json_output:
                ...
                click.echo(formatter.format_file_statistics(log_file_path, stats, len(entries)))
        else:
            # Show full results
            if json_output:
                output = {
        ...
        CLIErrorHandler.handle_file_error(e, json_output, log_file_path, "log file parsing")
...
def analyze_log_file(log_file_path: str, json_output: bool):
    ...
    try:
        ...
        CLIErrorHandler.handle_file_error(e, json_output, log_file_path, "log file analysis")
...
def validate_log_file(log_file_path: str, json_output: bool):
    ...
    try:
        ...
        CLIErrorHandler.handle_file_error(e, json_output, log_file_path, "log file validation")