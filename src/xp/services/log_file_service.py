...
from ..utils.time_utils import parse_log_timestamp, TimeParsingError, calculate_duration_ms
...
class LogFileService:
    ...
    def parse_log_file(self, file_path: str, base_date: Optional[datetime] = None) -> List[LogEntry]:
        """
    ...
    def parse_log_lines(self, lines: List[str], base_date: Optional[datetime] = None) -> List[LogEntry]:
        """
    ...
    def _parse_log_line(self, line: str, line_number: int, base_date: Optional[datetime] = None) -> Optional[LogEntry]:
        """
    ...
    def get_file_statistics(self, entries: List[LogEntry]) -> Dict[str, Any]:
        ...
        duration_ms = calculate_duration_ms(start_time, end_time) if start_time and end_time else 0
        ...
        return {
            ...
            "parse_success_rate": (valid_parses / total_entries * 100) if total_entries > 0 else 0,
            "direction_counts": {
                "tx": tx_count,
                "rx": rx_count
            },
            ...
            "checksum_validation": {
                ...
                "validation_success_rate": (valid_checksums / len(validated_entries) * 100) if validated_entries else 0
            },
            "time_range": {
                "start": start_time.strftime("%H:%M:%S.%f")[:-3] if start_time else None,
