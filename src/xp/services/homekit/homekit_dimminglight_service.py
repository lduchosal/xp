...
class HomeKitDimmingLightService:
    ...
    def handle_dimminglight_set_on(self, event: DimmingLightSetOnEvent) -> None:
        ...
        self.logger.info(f"Setting on light for serial {event.serial_number}, output {event.output_number} to {'ON' if event.value else 'OFF'} (brightness: {value})")
    ...
    def handle_dimminglight_set_brightness(self, event: DimmingLightSetBrightnessEvent) -> None:
        self.logger.info(f"Setting dimming light brightness"
    ...
    def handle_dimminglight_get_brightness(self, event: DimmingLightGetBrightnessEvent) -> None:
        self.logger.info(f"Getting dimming light brightness "
