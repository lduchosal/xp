...
class LinkNumberService:
    ...
    def generate_set_link_number_telegram(self, serial_number: str, link_number: int) -> str:
        """
        ...
        if not serial_number or len(serial_number) != 10:
            raise LinkNumberError(f"Serial number must be 10 digits, got: {serial_number}")
        
        if not serial_number.isdigit():
            raise LinkNumberError(f"Serial number must contain only digits: {serial_number}")
        
        # Validate link number range
        if not (0 <= link_number <= 99):
            raise LinkNumberError(f"Link number must be between 0-99, got: {link_number}")
    ...
    def create_set_link_number_telegram_object(self, serial_number: str, link_number: int) -> SystemTelegram:
        """
        ...
        Raises:
            LinkNumberError: If parameters are invalid
        """
        raw_telegram = self.generate_set_link_number_telegram(serial_number, link_number)
    ...
    def create_read_link_number_telegram_object(self, serial_number: str) -> SystemTelegram:
        """
    ...
    def parse_link_number_from_reply(self, reply_telegram: ReplyTelegram) -> Optional[int]:
        """
        ...
        """
        if (reply_telegram.data_point_id != DataPointType.LINK_NUMBER or
            not reply_telegram.data_value):
            return None
