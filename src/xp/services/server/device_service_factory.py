...
class DeviceServiceFactory:
    ...
    def create_device(self, module_type: str, serial_number: str) -> BaseServerService:
        ...
        if module_type == "CP20":
            device: BaseServerService = CP20ServerService(
                serial_number, "CP20", self.ms_serializer
            )
        elif module_type == "XP24":
            device = XP24ServerService(serial_number, "XP24", self.xp24ms_serializer)
        elif module_type in {"XP33", "XP33LR", "XP33LED"}:
            device = XP33ServerService(
                serial_number, module_type, self.xp33ms_serializer
            )
        elif module_type == "XP20":
            device = XP20ServerService(serial_number, "XP20", self.xp20ms_serializer)
        elif module_type == "XP130":
            device = XP130ServerService(serial_number, "XP130", self.ms_serializer)
        elif module_type == "XP230":
            device = XP230ServerService(serial_number, "XP230", self.ms_serializer)
        else:
            msg = f"Unknown device type '{module_type}' for serial {serial_number}"
