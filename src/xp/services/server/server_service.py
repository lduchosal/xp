...
class ServerService:
    ...
    def __init__(
        ...
        Args:
            ...
            device_factory: Factory for creating device service instances
                (injected via DI).
    ...
    def _load_device_config(self) -> None:
        ...
        try:
            if Path(self.config_path).exists():
                ...
                self.logger.info("Loaded %s devices from config", len(self.devices))
            else:
                self.logger.warning(
                    "Config file %s not found, using empty device list",
                    self.config_path,
                )
                self.devices = []
                self.device_services = {}
        except Exception:
            self.logger.exception("Error loading config file")
            self.devices = []
            self.device_services = {}
        ...
        for module in self.devices:
            ...
            try:
                ...
                self.logger.exception(
                    "Error creating device service for %s", serial_number
                )
    ...
    def start_server(self) -> None:
        ...
        try:
            ...
            # Bind to port 10001 on all interfaces: the emulator must be
            # reachable from external Conbus clients by design.
            self.server_socket.bind(("0.0.0.0", self.port))  # noqa: S104
            self.server_socket.listen(1)  # Accept single connection as per spec

            self._start_device_collector_thread()

            self.is_running = True
            self.logger.info("Conbus emulator server started on port %s", self.port)
            self.logger.info(
                "Configured devices: %s",
                [device.serial_number for device in self.devices],
            )
    ...
    def _flush_client_queue(
        self,
        client_queue: "queue.Queue[str]",
        client_socket: socket.socket,
        client_address: tuple[str, int],
    ) -> None:
        """Send all waiting buffers from the client's dedicated queue."""
        while not client_queue.empty():
            try:
                buffer = client_queue.get_nowait()
                client_socket.send(buffer.encode("latin-1"))
                self.logger.debug("Sent buffer to %s", client_address)
            except queue.Empty:
                break
    ...
    def _handle_client(
        ...
        try:
            ...
            while True:
                ...
                self._flush_client_queue(client_queue, client_socket, client_address)
                ...
                self.logger.debug("Received from %s: %s", client_address, message)
            ...
            self.logger.debug("Client %s timed out", client_address)
        except Exception:
            self.logger.exception("Error handling client %s", client_address)
        finally:
            try:
                # Unregister client before closing socket
                self.client_buffers.unregister_client(client_socket)
                client_socket.close()
                self.logger.info("Client %s disconnected", client_address)
            except Exception:
                self.logger.exception("Error closing client socket")

    def _process_request(self, message: str) -> list[str]:
        """Process incoming request and generate responses.

        Args:
            message: Message potentially containing multiple telegrams in
                format <TELEGRAM><TELEGRAM2>...
        ...
        try:
            ...
            if not telegrams:
                self.logger.warning("No valid telegrams found in message: %s", message)
                return responses
    ...
    def _split_telegrams(self, message: str) -> list[str]:
        ...
        Args:
            message: Raw message containing one or more telegrams in
                format <TELEGRAM><TELEGRAM2>...
        ...
        while True:
            ...
            if end_idx == -1:
                self.logger.warning(
                    "Incomplete telegram found starting at position %s", start_idx
                )
                break
    ...
    def _process_single_telegram(self, telegram: str) -> list[str]:
        ...
        try:
            ...
            if not parsed_telegram:
                self.logger.warning("Failed to parse telegram: %s", telegram)
                return responses
            ...
            if self.discover_service.is_discover_request(parsed_telegram):
                ...
                elif serial_number in self.device_services:
                    device_service = self.device_services[serial_number]
                    device_response: str | None = (
                        device_service.process_system_telegram(parsed_telegram)
                    )
                    if device_response:
                        responses.append(f"{device_response}\n")
                else:
                    self.logger.debug("No device found for serial: %s", serial_number)

        except Exception:
            self.logger.exception("Error processing telegram")
    ...
    def get_server_status(self) -> dict:
        ...
        return {
            ...
            "device_list": [device.serial_number for device in self.devices],
        }
    ...
    def reload_config(self) -> None:
        ...
        self._load_device_config()
        self.logger.info(
            "Configuration reloaded: %s devices, %s services",
            len(self.devices),
            len(self.device_services),
        )
    ...
    def _stop_device_collector_thread(self) -> None:
        ...
        self.logger.info("Stopping collector thread: %s", self.collector_thread.name)
