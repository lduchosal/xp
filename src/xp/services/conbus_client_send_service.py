...
from ..models.conbus_client_send import (
...
class ConbusClientSendService:
    ...
    def connect(self) -> Response:
        ...
        try:
            ...
            self.logger.info(f"Connected to Conbus server at {self.config.ip}:{self.config.port}")
        ...
        try:
            if request.telegram_type == TelegramType.DISCOVERY:
                ...
                if not request.target_serial:
                    return None
                result = self.version_service.generate_version_request_telegram(request.target_serial)
                return result.data.get('telegram') if result.success else None
            
            elif request.telegram_type in [TelegramType.VOLTAGE, TelegramType.TEMPERATURE, 
                                          TelegramType.CURRENT, TelegramType.HUMIDITY]:
    ...
    def send_sensor_request(self, target_serial: str, sensor_type: TelegramType) -> ConbusSendResponse:
        """Send sensor data request telegram"""
        if sensor_type not in [TelegramType.VOLTAGE, TelegramType.TEMPERATURE, 
                              TelegramType.CURRENT, TelegramType.HUMIDITY]:
            raise ConbusClientSendError(f"Invalid sensor type: {sensor_type.value}")
        
        request = ConbusSendRequest(
            telegram_type=sensor_type,
            target_serial=target_serial
        )
        return self.send_telegram(request)
