...
class ModuleTypeService:
    ...
    @staticmethod
    def get_module_type(identifier: int | str) -> ModuleType:
        ...
        if isinstance(identifier, int):
            ...
            if not module_type:
                msg = f"Module type with code {identifier} not found"
                raise ModuleTypeNotFoundError(msg)
        else:
            module_type = ModuleType.from_name(identifier)
            if not module_type:
                msg = f"Module type with name '{identifier}' not found"
                raise ModuleTypeNotFoundError(msg)
    ...
    def search_modules(
        ...
        for module_type in get_all_module_types():
            ...
            if (
                "name" in search_fields and query_lower in module_type.name.lower()
            ) or (
    ...
    def get_module_info_summary(self, identifier: int | str) -> str:
    ...
    def _format_all_modules() -> str:
        ...
        lines.extend(
            f"{module.code:4} | {module.name:10} | {module.description}"
    ...
    def _format_modules_by_category() -> str:
        ...
        for category, modules in categories.items():
            lines.append(f"\n=== {category} ===")
            lines.extend(
                f"  {module.code:2} - {module.name}: {module.description}"
                for module in modules
            )
