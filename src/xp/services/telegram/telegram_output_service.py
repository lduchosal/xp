...
# Length of a module serial number
SERIAL_NUMBER_LENGTH = 10

# Number of output state bits in a module output state reply
OUTPUT_STATE_BITS = 4
...
class TelegramOutputService:
    ...
    def validate_serial_number(serial_number: str) -> None:
        ...
        """
        if len(serial_number) != SERIAL_NUMBER_LENGTH or not serial_number.isdigit():
            msg = (
    ...
    def parse_reply_telegram(self, raw_telegram: str) -> OutputTelegram:
        ...
        if not match:
            msg = f"Invalid XP24 response telegram format: {raw_telegram}"
            raise XPOutputError(msg)
        ...
        try:
            ...
            if system_function is None:
                msg = f"Unknown system_function: {ack_nak}"
                raise XPOutputError(msg)
            ...
            msg = f"Invalid values in XP24 action telegram: {e}"
            raise XPOutputError(msg) from e
    ...
    def parse_system_telegram(self, raw_telegram: str) -> OutputTelegram:
        ...
        if not match:
            msg = f"Invalid XP24 action telegram format: {raw_telegram}"
            raise XPOutputError(msg)
        ...
        try:
            ...
            if action_type is None:
                msg = f"Unknown action code: {action_code}"
                raise XPOutputError(msg)
    ...
    def parse_status_response(self, raw_telegram: str) -> list[bool]:
        ...
        Returns:
            Dictionary mapping output numbers (0-3) to their states
            (True=ON, False=OFF).
        ...
        """
        if not raw_telegram:
            msg = "Empty status response telegram"
            raise XPOutputError(msg)
        ...
        if not reply_telegram or not reply_telegram.data_value:
            msg = "Not a reply telegram"
            raise XPOutputError(msg)

        if (
            not reply_telegram.datapoint_type
            or reply_telegram.datapoint_type != DataPointType.MODULE_OUTPUT_STATE
        ):
            msg = "Not a DataPoint telegram"
            raise XPOutputError(msg)

        status_bits = reply_telegram.data_value.replace("xxxx", "")[::-1][
            0:OUTPUT_STATE_BITS
        ]
        if len(status_bits) != OUTPUT_STATE_BITS:
            msg = "Not a module_output_state telegram"
            raise XPOutputError(msg)

        status = [False, False, False, False]
        for i in range(OUTPUT_STATE_BITS):
    ...
    def format_output_state(data_value: str) -> str:
        ...
        Args:
            data_value: Raw data value from module output state datapoint
                (e.g., "xxxx0101", "xx1110").
