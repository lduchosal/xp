...
# Maximum link number on the console bus
MAX_LINK_NUMBER = 99

# Maximum input/output number on the console bus
MAX_INPUT_NUMBER = 99

# Length of a telegram checksum
CHECKSUM_LENGTH = 2

# Length of the "D{data_point}" header in a reply data value
REPLY_DATA_HEADER_LENGTH = 3
...
class TelegramService:
    ...
    def parse_event_telegram(self, raw_telegram: str) -> EventTelegram:
        ...
        try:
            ...
            if event_telegram_type not in {"E", "O"}:
                msg = f"Event telegram type (E or O): {event_telegram_type}"
                raise TelegramParsingError(msg)

            if not (0 <= link_number <= MAX_LINK_NUMBER):
                msg = f"Link number out of range (0-99): {link_number}"
                raise TelegramParsingError(msg)

            if not (0 <= output_number <= MAX_INPUT_NUMBER):
                msg = f"Input number out of range (0-99): {output_number}"
                raise TelegramParsingError(msg)
            ...
            try:
                ...
                msg = f"Invalid event type: {event_type_char}"
                raise TelegramParsingError(msg) from err
            ...
            msg = f"Invalid numeric values in telegram: {e}"
            raise TelegramParsingError(msg) from e

        return telegram

    @staticmethod
    def validate_checksum(
        telegram: EventTelegram | ReplyTelegram | SystemTelegram | OutputTelegram,
    ) -> bool:
        ...
        """
        if not telegram.checksum or len(telegram.checksum) != CHECKSUM_LENGTH:
            return False
    ...
    def parse_system_telegram(self, raw_telegram: str) -> SystemTelegram:
        ...
        if not match:
            msg = f"Invalid system telegram format: {raw_telegram}"
            raise TelegramParsingError(msg)
        ...
        try:
            ...
            if system_function is None:
                msg = f"Unknown system function code: {function_code}"
                raise TelegramParsingError(msg)
            ...
            msg = f"Invalid values in system telegram: {e}"
            raise TelegramParsingError(msg) from e
    ...
    def parse_reply_telegram(self, raw_telegram: str) -> ReplyTelegram:
        ...
        Args:
            raw_telegram: The raw telegram string
                (e.g., "<R0020012521F02D18+26,0§CIL>").
        ...
        # Validate and parse using regex
        self.logger.debug("Parsing reply telegram %s", raw_telegram)
        match = self.REPLY_TELEGRAM_PATTERN.match(raw_telegram.strip())
        if not match:
            msg = f"Invalid reply telegram format: {raw_telegram}"
            raise TelegramParsingError(msg)
        ...
        try:
            ...
            if (
                full_data_value.startswith("D")
                and len(full_data_value) >= REPLY_DATA_HEADER_LENGTH
            ):
                # Regular reply format: D{data_point}{data}
                data = full_data_value[1:REPLY_DATA_HEADER_LENGTH]
                data_value = full_data_value[REPLY_DATA_HEADER_LENGTH:]
            else:
            ...
            msg = f"Invalid values in reply telegram: {e}"
            raise TelegramParsingError(msg) from e
    ...
    def parse_telegram(
    ...
    ) -> EventTelegram | SystemTelegram | ReplyTelegram:
        ...
        if telegram_type_code == TelegramType.SYSTEM.value:
            return self.parse_system_telegram(raw_telegram)
        if telegram_type_code == TelegramType.REPLY.value:
            return self.parse_reply_telegram(raw_telegram)
        msg = f"Unknown telegram type code: {telegram_type_code}"
        raise TelegramParsingError(msg)
