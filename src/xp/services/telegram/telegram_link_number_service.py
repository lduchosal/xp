...
# Maximum configurable link number
MAX_LINK_NUMBER = 99
...
class LinkNumberService:
    ...
    def generate_set_link_number_telegram(serial_number: str, link_number: int) -> str:
        ...
        if not serial_number or len(serial_number) != SERIAL_NUMBER_LENGTH:
            msg = f"Serial number must be 10 digits, got: {serial_number}"
            raise LinkNumberError(msg)

        if not serial_number.isdigit():
            msg = f"Serial number must contain only digits: {serial_number}"
            raise LinkNumberError(msg)

        # Validate link number range
        if not (0 <= link_number <= MAX_LINK_NUMBER):
            msg = f"Link number must be between 0-99, got: {link_number}"
            raise LinkNumberError(msg)
    ...
    @staticmethod
    def parse_link_number_from_reply(reply_telegram: ReplyTelegram) -> int | None:
        ...
        with suppress(ValueError, TypeError):
            ...
            if 0 <= link_number <= MAX_LINK_NUMBER:
                return link_number
