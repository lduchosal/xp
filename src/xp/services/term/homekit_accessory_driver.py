...
class OnSetCallback(Protocol):
    """Callback invoked when the HomeKit app changes accessory state."""

    def __call__(
        self, accessory_name: str, *, is_on: bool, brightness: int | None
    ) -> None:
        """Handle a HomeKit set event.

        Args:
            accessory_name: Accessory name (internal identifier).
...
class XPAccessory(Accessory):
    ...
    def __init__(
        ...
        """
        super().__init__(driver.hap_driver, display_name, aid=aid)
        self._hk_driver = driver
        self._accessory_id = name
        self._is_dimmable = service_type == "dimminglight"
        self._char_brightness: object | None = None
    ...
    # pyhap invokes setter_callback positionally with the characteristic value;
    # the parameter cannot be keyword-only.
    def _set_on(self, value: bool) -> None:  # noqa: FBT001
        ...
        """
        self._hk_driver.notify_set(self._accessory_id, is_on=value, brightness=None)
    ...
    def _set_brightness(self, value: int) -> None:
        ...
        """
        self._hk_driver.notify_set(self._accessory_id, is_on=True, brightness=value)
        self._current_brightness = value

    def update_state(self, *, is_on: bool, brightness: int | None = None) -> None:
...
class HomekitAccessoryDriver:
    ...
    def __init__(self, homekit_config: HomekitConfig) -> None:
        ...
        self._on_set: OnSetCallback | None = None

    @property
    def hap_driver(self) -> AccessoryDriver | None:
        """The underlying pyhap AccessoryDriver."""
        return self._driver

    def notify_set(
        self, accessory_id: str, *, is_on: bool, brightness: int | None
    ) -> None:
        """Invoke the registered on-set callback, if any.

        Args:
            accessory_id: Accessory name (internal identifier).
            is_on: True for on, False for off.
            brightness: Brightness value 0-100, or None for on/off only.

        """
        if self._on_set:
            self._on_set(accessory_id, is_on=is_on, brightness=brightness)

    def set_callback(self, on_set: OnSetCallback) -> None:
        """Set callback for HomeKit set events.

        Args:
            on_set: Callback(accessory_name, is_on, brightness) called when
                the HomeKit app changes state. brightness is None for on/off
                only, or 0-100 for dimming.
    ...
    def _setup_bridge(self, config: HomekitConfig) -> None:
        ...
        Raises:
            RuntimeError: If the AccessoryDriver is not initialized.

        """
        if self._driver is None:
            msg = "AccessoryDriver is not initialized"
            raise RuntimeError(msg)
        bridge = Bridge(self._driver, config.bridge.name)

        # Bridge is aid 1; accessories start at 2
        for aid, acc_config in enumerate(config.accessories, start=2):
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "Starting HAP driver on %s:%s with pincode %s",
                config.homekit.ip,
                config.homekit.port,
                config.homekit.pincode,
            )
            ...
            self.logger.exception("Error starting AccessoryDriver")
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            self.logger.exception("Error stopping AccessoryDriver")

    def update_state(
        self, accessory_name: str, *, is_on: bool, brightness: int | None = None
    ) -> None:
        """Update accessory state from Conbus event.
        ...
        if acc:
            acc.update_state(is_on=is_on, brightness=brightness)
        else:
            self.logger.warning("Unknown accessory name: %s", accessory_name)
