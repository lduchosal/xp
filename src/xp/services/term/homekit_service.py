...
class HomekitService:
    ...
    def __init__(
        ...
        self._active_level_action: tuple[str, str] | None = None
        ...
        for room in self._homekit_config.bridge.rooms:
            for accessory_name in room.accessories:
                ...
                if not accessory_config:
                    self.logger.warning(
                        "Accessory config not found for %s", accessory_name
                    )
                    continue
    ...
    def _find_accessory_config(self, name: str) -> HomekitAccessoryConfig | None:
    ...
    def _on_homekit_set(
        ...
        if not config:
            self.logger.warning("No config found for accessory: %s", accessory_name)
            return
    ...
    def _handle_brightness_change(
        ...
        if delta > 0:
            ...
            if not config.dimup_action:
                self.logger.warning("No dimup_action for %s", accessory_name)
                return
            ...
            if not config.dimdown_action:
                self.logger.warning("No dimdown_action for %s", accessory_name)
                return
        ...
        self.on_status_message.emit(
            f"HomeKit: {accessory_name} {current}% → {target_brightness}% "
            f"({direction}{steps * step_size}%)"
        )
    ...
    def select_accessory(self, action_key: str) -> str | None:
    ...
    def toggle_selected(self, accessory_id: str) -> bool:
        ...
        if not state or not state.toggle_action:
            self.logger.warning("No toggle_action for accessory %s", accessory_id)
            return False
    ...
    def turn_on_selected(self, accessory_id: str) -> bool:
        ...
        if not config or not state:
            self.logger.warning("No config for accessory %s", accessory_id)
            return False
        ...
        for state in self._accessory_states.values():
            if state.serial_number == serial_number:
                ...
                if output_index < len(output_list):
                    ...
                    if config:
                        self._accessory_driver.update_state(config.name, is_on=is_on)
                else:
                    state.output_state = "?"

                state.last_update = local_now()
                self.on_module_state_changed.emit(state)

    @staticmethod
    def _event_output_number(module_type: int, input_number: int) -> int | None:
        """Determine the output number reported by an event telegram.

        Args:
            module_type: Module type code from the event telegram.
            input_number: Input number from the event telegram.

        Returns:
            0-based output number, or None if the event is not an output
            status event for a supported module type.

        """
        if module_type == ModuleTypeCode.XP24.value:
            input_max = XP24_OUTPUT_EVENT_INPUT_MAX
        elif module_type in {
            ModuleTypeCode.XP33.value,
            ModuleTypeCode.XP33LR.value,
            ModuleTypeCode.XP33LED.value,
        }:
            input_max = XP33_OUTPUT_EVENT_INPUT_MAX
        else:
            return None

        if OUTPUT_EVENT_INPUT_BASE <= input_number <= input_max:
            return input_number - OUTPUT_EVENT_INPUT_BASE
        return None
    ...
    def _handle_event_telegram(self, event: TelegramReceivedEvent) -> None:
        ...
        output_number = self._event_output_number(
            event_telegram.module_type, event_telegram.input_number
        )
